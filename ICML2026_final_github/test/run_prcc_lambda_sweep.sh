...
NUM_LAMBDAS=3
NUM_TRAIN_CASES=$((NUM_SCAN_PER_LAMBDA * NUM_LAMBDAS))
NUM_DEFAULT_RECORDS=${NUM_LAMBDAS}
NUM_TOTAL_ROWS=$((NUM_TRAIN_CASES + NUM_DEFAULT_RECORDS))

print_help() {
  cat << EOF
用法:
  bash run_prcc_lambda_sweep.sh [选项] [-- 传给 continual_train.py 的其他参数]
...
示例:
  bash run_prcc_lambda_sweep.sh --gpus 1
  bash run_prcc_lambda_sweep.sh --gpus 0,1 -- --epochs0 40 --epochs 30 --middle_test
EOF
}
...
RUN_BASE="${LOG_ROOT%/}/prcc_lambda_sweep_${RUN_TAG}"
MASTER_LOG="${RUN_BASE}/prcc_lambda_sweep_master.log"
CONFIG_TSV="${RUN_BASE}/runs_config.tsv"
REPORT_MD="${RUN_BASE}/prcc_lambda_sweep_report.md"

mkdir -p "${RUN_BASE}"

echo "==========================================" | tee -a "${MASTER_LOG}"
echo "PRCC Lambda 消融实验开始: $(date)" | tee -a "${MASTER_LOG}"
